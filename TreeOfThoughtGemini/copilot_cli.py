...
try:
    ...
    pass # readline is not available on all platforms, but it's not critical
import shlex
...
class ACPClient:
    ...
    to the Copilot CLI via JSON-RPC 2.0 over standard input/output.
    """
    def __init__(self, cli_args: List[str] = None):
        if cli_args is None:
            # Enable ACP mode natively for Copilot
            if os.name == 'nt':
                cli_args = ["cmd.exe", "/c", "copilot --acp --yolo --model gpt-5-mini"]
            else:
                cli_args = ["copilot", "--acp", "--yolo", "--model gpt-5-mini"]
        ...
        try:
            ...
            raise ACPClientError("Failed to execute Copilot CLI. Ensure 'copilot' is in the system PATH.")
    ...
    def _authenticate_protocol(self):
        ...
        For Copilot, authentication is handled out-of-band via its CLI context.
        """
        # Kept as a pass-through in case Copilot adds strict ACP auth requirements later.
        pass
...
def main():
    ...
    print("==================================================")
    print("         Copilot ACP Chat & ToT Harness          ")
    print("==================================================")
